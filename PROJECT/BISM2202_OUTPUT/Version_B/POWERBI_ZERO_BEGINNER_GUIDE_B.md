...
**标题：** `Seasonal Pattern in Order Volume`
...
7. 在 Format visual > General > Title 开启标题并输入 `Seasonal Pattern in Order Volume`。
...
**目标 Visual：** 100% stacked bar chart + slicer  
**字段槽位：** Y-axis: Payment Method; Legend: Traffic Level; X-axis: [Order Count]; slicer: Order Time  
...
2. 在 Visualizations/Build visual 中选择 **100% stacked bar chart + slicer**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**Y-axis: Payment Method; Legend: Traffic Level; X-axis: [Order Count]; slicer: Order Time**。Measure 带计算器图标，普通列没有。
...
**目标 Visual：** Dashboard: bar + column + 100% stacked bar  
**字段槽位：** Top locations; Pizza type delivery time; Traffic mix by payment  
**筛选/排序/格式：** Order Time slicer; three coordinated visuals  
