...
**字段槽位：** Y-axis: Location; X-axis: [Order Count]  
**筛选/排序/格式：** Top N = 20 by [Order Count]; descending  
**标题：** `Top 20 Locations by Order Volume`

1. 在底部单击页面 `Q01`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Clustered bar chart**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**Y-axis: Location; X-axis: [Order Count]**。Measure 带计算器图标，普通列没有。
...
7. 在 Format visual > General > Title 开启标题并输入 `Top 20 Locations by Order Volume`。
...
**字段槽位：** X-axis: Pizza Size; Y-axis: [Avg Delivery Duration]  
**筛选/排序/格式：** Data labels on; 1 decimal  
**标题：** `Average Delivery Duration by Pizza Size`

1. 在底部单击页面 `Q02`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Clustered column chart**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**X-axis: Pizza Size; Y-axis: [Avg Delivery Duration]**。Measure 带计算器图标，普通列没有。
...
6. 检查轴标题、图例和数据标签没有遮挡；必要时拉宽 Visual，不要缩小到看不清字段名。
7. 在 Format visual > General > Title 开启标题并输入 `Average Delivery Duration by Pizza Size`。
...
**目标 Visual：** Donut chart  
**字段槽位：** Legend: Pizza Size; Values: [Order Count]  
**筛选/排序/格式：** Detail labels: category + percent of total  
**标题：** `Share of Orders by Pizza Size`

1. 在底部单击页面 `Q03`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Donut chart**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**Legend: Pizza Size; Values: [Order Count]**。Measure 带计算器图标，普通列没有。
4. 设置 Visual：Detail labels: category + percent of total。若字段默认显示 Sum，点字段槽位右侧下拉箭头，改用指定 Measure 或 Average/Distinct count。
5. 点 Visual 右上 `...`，按题意完成排序；然后在 Format visual 中把 Data labels 开启，并按需要设置 1–2 位小数。
6. 在 Format visual > Detail labels/Data labels 中开启 Category 和 Percent of total；若选项名称不同，展开 Labels 的内容选项，确保截图同时能看出类别和百分比。
7. 在 Format visual > General > Title 开启标题并输入 `Share of Orders by Pizza Size`。
...
**目标 Visual：** Clustered column chart  
**字段槽位：** X-axis: Payment Method; Y-axis: [Order Count]  
**筛选/排序/格式：** Descending by [Order Count]  
**标题：** `Orders by Payment Method`

1. 在底部单击页面 `Q04`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Clustered column chart**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**X-axis: Payment Method; Y-axis: [Order Count]**。Measure 带计算器图标，普通列没有。
4. 设置 Visual：Descending by [Order Count]。若字段默认显示 Sum，点字段槽位右侧下拉箭头，改用指定 Measure 或 Average/Distinct count。
5. 点 Visual 右上 `...`，按题意完成排序；然后在 Format visual 中把 Data labels 开启，并按需要设置 1–2 位小数。
6. 检查轴标题、图例和数据标签没有遮挡；必要时拉宽 Visual，不要缩小到看不清字段名。
7. 在 Format visual > General > Title 开启标题并输入 `Orders by Payment Method`。
...
**目标 Visual：** Donut chart  
**字段槽位：** Legend: Traffic Level; Values: [Order Count]  
**筛选/排序/格式：** Category + percentage labels  
**标题：** `Share of Orders by Traffic Level`

1. 在底部单击页面 `Q05`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Donut chart**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**Legend: Traffic Level; Values: [Order Count]**。Measure 带计算器图标，普通列没有。
4. 设置 Visual：Category + percentage labels。若字段默认显示 Sum，点字段槽位右侧下拉箭头，改用指定 Measure 或 Average/Distinct count。
5. 点 Visual 右上 `...`，按题意完成排序；然后在 Format visual 中把 Data labels 开启，并按需要设置 1–2 位小数。
6. 在 Format visual > Detail labels/Data labels 中开启 Category 和 Percent of total；若选项名称不同，展开 Labels 的内容选项，确保截图同时能看出类别和百分比。
7. 在 Format visual > General > Title 开启标题并输入 `Share of Orders by Traffic Level`。
...
**目标 Visual：** Clustered column chart  
**字段槽位：** X-axis: Weekend Label; Y-axis: [Avg Toppings Count]  
**筛选/排序/格式：** Data labels on; 2 decimals  
**标题：** `Average Toppings Count: Weekend vs Weekday`

1. 在底部单击页面 `Q06`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Clustered column chart**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**X-axis: Weekend Label; Y-axis: [Avg Toppings Count]**。Measure 带计算器图标，普通列没有。
4. 设置 Visual：Data labels on; 2 decimals。若字段默认显示 Sum，点字段槽位右侧下拉箭头，改用指定 Measure 或 Average/Distinct count。
5. 点 Visual 右上 `...`，按题意完成排序；然后在 Format visual 中把 Data labels 开启，并按需要设置 1–2 位小数。
6. 检查轴标题、图例和数据标签没有遮挡；必要时拉宽 Visual，不要缩小到看不清字段名。
7. 在 Format visual > General > Title 开启标题并输入 `Average Toppings Count: Weekend vs Weekday`。
...
**目标 Visual：** Clustered bar chart  
**字段槽位：** Y-axis: Location; X-axis: [Avg Delivery Duration]  
**筛选/排序/格式：** Top N = 10 by [Avg Delivery Duration]; descending; retain ties  
**标题：** `Locations with Highest Average Delivery Duration`

1. 在底部单击页面 `Q07`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Clustered bar chart**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**Y-axis: Location; X-axis: [Avg Delivery Duration]**。Measure 带计算器图标，普通列没有。
...
6. 检查轴标题、图例和数据标签没有遮挡；必要时拉宽 Visual，不要缩小到看不清字段名。
7. 在 Format visual > General > Title 开启标题并输入 `Locations with Highest Average Delivery Duration`。
...
**目标 Visual：** Donut chart  
**字段槽位：** Legend: Peak Hour Label; Values: [Order Count]  
**筛选/排序/格式：** Category + percentage labels  
**标题：** `Peak vs Non-Peak Order Volume`

1. 在底部单击页面 `Q08`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Donut chart**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**Legend: Peak Hour Label; Values: [Order Count]**。Measure 带计算器图标，普通列没有。
4. 设置 Visual：Category + percentage labels。若字段默认显示 Sum，点字段槽位右侧下拉箭头，改用指定 Measure 或 Average/Distinct count。
5. 点 Visual 右上 `...`，按题意完成排序；然后在 Format visual 中把 Data labels 开启，并按需要设置 1–2 位小数。
6. 在 Format visual > Detail labels/Data labels 中开启 Category 和 Percent of total；若选项名称不同，展开 Labels 的内容选项，确保截图同时能看出类别和百分比。
7. 在 Format visual > General > Title 开启标题并输入 `Peak vs Non-Peak Order Volume`。
...
**目标 Visual：** Clustered column chart  
**字段槽位：** X-axis: Pizza Size; Y-axis: [Avg Toppings Count]  
**筛选/排序/格式：** Data labels on; 2 decimals  
**标题：** `Average Toppings Count by Pizza Size`

1. 在底部单击页面 `Q10`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Clustered column chart**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**X-axis: Pizza Size; Y-axis: [Avg Toppings Count]**。Measure 带计算器图标，普通列没有。
4. 设置 Visual：Data labels on; 2 decimals。若字段默认显示 Sum，点字段槽位右侧下拉箭头，改用指定 Measure 或 Average/Distinct count。
5. 点 Visual 右上 `...`，按题意完成排序；然后在 Format visual 中把 Data labels 开启，并按需要设置 1–2 位小数。
6. 检查轴标题、图例和数据标签没有遮挡；必要时拉宽 Visual，不要缩小到看不清字段名。
7. 在 Format visual > General > Title 开启标题并输入 `Average Toppings Count by Pizza Size`。
...
**目标 Visual：** Clustered bar chart  
**字段槽位：** Y-axis: Restaurant Name; X-axis: [Avg Delay]  
**筛选/排序/格式：** Visual filter [Delay Rank Asc] <= 2; ascending  
**标题：** `Two Restaurants with Lowest Average Delay`

1. 在底部单击页面 `Q11`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Clustered bar chart**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**Y-axis: Restaurant Name; X-axis: [Avg Delay]**。Measure 带计算器图标，普通列没有。
...
6. 检查轴标题、图例和数据标签没有遮挡；必要时拉宽 Visual，不要缩小到看不清字段名。
7. 在 Format visual > General > Title 开启标题并输入 `Two Restaurants with Lowest Average Delay`。
...
**目标 Visual：** Clustered bar chart  
**字段槽位：** Y-axis: Pizza Type; X-axis: [Avg Delivery Duration]  
**筛选/排序/格式：** Top N = 5 by [Avg Delivery Duration]; descending  
**标题：** `Top 5 Pizza Types by Average Delivery Duration`

1. 在底部单击页面 `Q12`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Clustered bar chart**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**Y-axis: Pizza Type; X-axis: [Avg Delivery Duration]**。Measure 带计算器图标，普通列没有。
...
5. 点 Visual 右上 `...` > Sort by [Avg Delivery Duration] > Descending。若 Q7 因并列显示超过 10 个位置，这是正常的 tie 行为，两个版本必须保留相同并列项。
6. 检查轴标题、图例和数据标签没有遮挡；必要时拉宽 Visual，不要缩小到看不清字段名。
7. 在 Format visual > General > Title 开启标题并输入 `Top 5 Pizza Types by Average Delivery Duration`。
...
**筛选/排序/格式：** Order Hour ascending; data markers on  
**标题：** `Hourly Order Volume`
...
4. 设置 Visual：Order Hour ascending; data markers on。若字段默认显示 Sum，点字段槽位右侧下拉箭头，改用指定 Measure 或 Average/Distinct count。
5. 点 Visual 右上 `...`，按题意完成排序；然后在 Format visual 中把 Data labels 开启，并按需要设置 1–2 位小数。
6. 确认 X-axis 使用 Whole Number `Order Hour`，不是把它当日期或文本；排序必须是 12、13、14、17、18、19、20、21。
7. 在 Format visual > General > Title 开启标题并输入 `Hourly Order Volume`。
...
**目标 Visual：** Clustered column chart  
**字段槽位：** X-axis: Traffic Level; Y-axis: [Avg Delay]  
**筛选/排序/格式：** Low/Medium/High risk colors; data labels on  
**标题：** `Average Delay by Traffic Level`

1. 在底部单击页面 `Q14`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Clustered column chart**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**X-axis: Traffic Level; Y-axis: [Avg Delay]**。Measure 带计算器图标，普通列没有。
4. 设置 Visual：Low/Medium/High risk colors; data labels on。若字段默认显示 Sum，点字段槽位右侧下拉箭头，改用指定 Measure 或 Average/Distinct count。
5. 点 Visual 右上 `...`，按题意完成排序；然后在 Format visual 中把 Data labels 开启，并按需要设置 1–2 位小数。
6. 检查轴标题、图例和数据标签没有遮挡；必要时拉宽 Visual，不要缩小到看不清字段名。
7. 在 Format visual > General > Title 开启标题并输入 `Average Delay by Traffic Level`。
...
**目标 Visual：** Donut chart  
**字段槽位：** Legend: Pizza Complexity; Values: [Order Count]  
**筛选/排序/格式：** Treat numeric field as category; percentage labels  
**标题：** `Order Share by Pizza Complexity`

1. 在底部单击页面 `Q15`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Donut chart**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**Legend: Pizza Complexity; Values: [Order Count]**。Measure 带计算器图标，普通列没有。
4. 设置 Visual：Treat numeric field as category; percentage labels。若字段默认显示 Sum，点字段槽位右侧下拉箭头，改用指定 Measure 或 Average/Distinct count。
5. 点 Visual 右上 `...`，按题意完成排序；然后在 Format visual 中把 Data labels 开启，并按需要设置 1–2 位小数。
6. 在 Format visual > Detail labels/Data labels 中开启 Category 和 Percent of total；若选项名称不同，展开 Labels 的内容选项，确保截图同时能看出类别和百分比。
7. 在 Format visual > General > Title 开启标题并输入 `Order Share by Pizza Complexity`。
...
**目标 Visual：** Table  
**字段槽位：** Columns: Restaurant Name, [Avg Delivery Duration], [Avg Delay]  
**筛选/排序/格式：** Background color Fx > Field value using two color measures  
**标题：** `Restaurant Delivery Performance`

1. 在底部单击页面 `Q16`，再单击画布空白处，确保没有选中别的 Visual。
2. 在 Visualizations/Build visual 中选择 **Table**。若图标名称不确定，把鼠标停在图标上等待工具提示。
3. 从 Data pane 展开 `PizzaOrders`，按以下位置逐个拖字段：**Columns: Restaurant Name, [Avg Delivery Duration], [Avg Delay]**。Measure 带计算器图标，普通列没有。
...
5. Conditional formatting 对话框中 Format style 选 Field value；What field should we base this on/Based on field 选 `[Delivery Duration Color A]`；Summarization 选 First；点 OK。
6. 对 `[Avg Delay]` 重复：Background color > Fx > Format style = Field value > Based on field = `[Delay Color A]` > OK。确认不是手工逐格上色。
6. 检查轴标题、图例和数据标签没有遮挡；必要时拉宽 Visual，不要缩小到看不清字段名。
7. 在 Format visual > General > Title 开启标题并输入 `Restaurant Delivery Performance`。
...
**标题：** `Hourly Order Volume and Average Delay`
...
7. 在 Format visual > General > Title 开启标题并输入 `Hourly Order Volume and Average Delay`。
...
**筛选/排序/格式：** Secondary Y-axis on; descending by [Order Count]  
**标题：** `Pizza Type Topping Density and Order Volume`
...
4. 设置 Visual：Secondary Y-axis on; descending by [Order Count]。若字段默认显示 Sum，点字段槽位右侧下拉箭头，改用指定 Measure 或 Average/Distinct count。
5. 点 Visual 右上 `...`，按题意完成排序；然后在 Format visual 中把 Data labels 开启，并按需要设置 1–2 位小数。
6. 在 Format visual 展开 Secondary y-axis，设为 On，并开启左右轴标题；Order Count 使用整数轴，平均值使用 1–2 位小数。
7. 在 Format visual > General > Title 开启标题并输入 `Pizza Type Topping Density and Order Volume`。
...
6. 检查轴标题、图例和数据标签没有遮挡；必要时拉宽 Visual，不要缩小到看不清字段名。
7. 在 Format visual > General > Title 开启标题并输入 `Traffic Mix within Payment Method`。
...
2. File > Save As，把 PBIX 保存到 `Version_A/BISM2202_Assignment_A.pbix`。关闭后重新打开一次，确认数据和 Visual 没有错误。
...
5. 打开 `BISM2202_Report_A.docx`，逐题核对截图数字、标题和英文 observation 完全一致。
